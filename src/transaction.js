...
Transaction.SIGHASH_BCHBIP143 = 0x40
...
/**
 * Hash transaction for signing a specific input for Bch.
 */
Transaction.prototype.hashForBchSignature = function (inIndex, prevOutScript, inAmount, hashType) {
  typeforce(types.tuple(types.UInt32, types.Buffer, /* types.UInt8 */ types.Number, types.maybe(types.UInt53)), arguments)

  // This function works the way it does because Bch
  ...
  // BIP143 sighash activated in Bch via 0x40 bit
  if (hashType & Transaction.SIGHASH_BCHBIP143) {
    if (types.Null(inAmount)) {
      throw new Error('Bch sighash requires value of input to be signed.')
    }
